// ========== PRO BASE (shared across every page except home.html) ==========
...
// knows nothing about the pro-pill or quote rotator — this file is the
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // [data-plan="pro"] styling in styles.css. Works the same for a
    // guest (user is null, isPro() resolves false) as for an account.
    let pro = false;
    if (window.isPro) {
        pro = await isPro();
        document.documentElement.setAttribute("data-plan", pro ? "pro" : "free");
    }
...
// Free-plan version: one quote, no rotation, no interval — the "normal"
// quote system, picked by the day so it still changes daily without
// the Pro carousel animation.
